...
export const ResultsView: React.FC<ResultsViewProps> = ({
  ...
  useEffect(() => {
    const fetchBudget = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error('Failed to fetch RAG budget');
        }
  ...
  const handleGenerateVideo = async () => {
    ...
    console.log('  originalImage type:', typeof originalImage);
    console.log('  originalImage value:', originalImage?.substring(0, 100));
    console.log('  propOriginalImage:', propOriginalImage);
    console.log('  yardImagePreview:', yardImagePreview);

    if (!originalImage || !result.renderImages[currentRenderIndex]) {
      console.error('❌ Missing required images for video generation');
      console.error('  originalImage present:', !!originalImage);
      console.error('  renderImage present:', !!result.renderImages[currentRenderIndex]);
      return;
    }
